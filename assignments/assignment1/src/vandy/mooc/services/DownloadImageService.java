...
/**
 * An IntentService that downloads an image requested via data in an intent, stores the image in a local file on the
 * local device, and returns the image file's URI back to the MainActivity via the Messenger passed with the intent.
 */
public class DownloadImageService extends IntentService {
	...
	/**
	 * Helper method that returns the path to the image file if it is download successfully.
	 */
	...
	/**
	 * Helper method that returns the request code associated with the @a message.
	 */
	...
	/**
	 * Hook method dispatched by the IntentService framework to download the image requested via data in an intent,
	 * store the image in a local file on the local device, and return the image file's URI back to the MainActivity via
	 * the Messenger passed with the intent.
	 */
	@Override
	public void onHandleIntent(Intent intent) {
		// Get the URL associated with the Intent data.
		Uri remoteUrl = intent.getData();

		// Get the directory pathname where the image will be stored.
		String dir = intent.getStringExtra(DIRECTORY_PATHNAME);

		// Download the requested image.
		Uri localUrl = Utils.downloadImage(getApplicationContext(), remoteUrl, dir);
		...
		sendPath(messenger, localUrl, remoteUrl);
	}
	...
	/**
	 * A factory method that creates a Message to return to the MainActivity with the pathname of the downloaded image.
	 */
	private Message makeReplyMessage(Uri pathToImageFile, Uri url) {
