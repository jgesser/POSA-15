...
/**
 * This class implements all the image-related operations using an Android Bound Service. It plays the role of the
 * "Concrete Implementor" in the Bridge pattern and the "Concrete Class" in the Template Method pattern.
 */
public class ImageOpsBoundService extends ImageOpsImpl {
	/**
	 * Reference to the reply Messenger that's passed to the DownloadImagesBoundService and used to return image results
	 * via the ServiceResultHandler.
	 */
	private Messenger mReplyMessenger = null;

	/**
	 * Reference to the request Messenger that's implemented in the DownloadImagesBoundService and used to send request
	 * messages to the Service.
	 */
	private Messenger mRequestMessengerRef = null;

	/**
	 * Used to receive a reference to the RequestMessenger after binding to the DownloadImagesBoundService using
	 * bindService().
	 */
	private final ServiceConnection mServiceConnection = new ServiceConnection() {
		/**
		 * Called by the Android Binder framework after the DownloadImagesBoundService is connected to convey the result
	...
	public void bindService() {
		if (mRequestMessengerRef == null) {
			...
			// TODOne -- you fill in here.
			mActivity.get().bindService(intent, mServiceConnection, Activity.BIND_AUTO_CREATE);
	...
	public void unbindService() {
		if (mRequestMessengerRef != null) {
			...
			// TODOno -- you fill in here.
			mRequestMessengerRef = null;
		}
	}

	/**
	 * Start a download. Plays the fole of the "Primitive Operation" (a.k.a., "Hook Method") in the Template Method
	 * pattern.
	 */
	@Override
	protected void startDownload(Uri url) {
		if (mRequestMessengerRef == null) {
			...
			try {
				...
				RequestMessage requestMessage = RequestMessage.makeRequestMessage(OperationType.DOWNLOAD_IMAGE.ordinal(), url, mDirectoryPathname, mReplyMessenger);
