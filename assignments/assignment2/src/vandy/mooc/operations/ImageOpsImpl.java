...
/**
 * This abstract class defines all the image-related operations. It plays the role of the "Implementor" in Bridge
 * pattern and the role of the "Abstract Class" in the Template Method pattern.
 */
public abstract class ImageOpsImpl {
	...
	/**
	 * Stores the running total number of images downloaded that must be handled by ServiceResultHandler.
	 */
	protected int mNumImagesToHandle;

	/**
	 * Stores the running total number of images that have been handled by the ServiceResultHandler.
	 */
	...
	/**
	 * Array of Strings that represent the valid URLs that have been entered.
	 */
	...
	public ImageOpsImpl(MainActivity activity) {
		...
		mDirectoryPathname = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM) + "/" + timestamp + "/";
	...
	/**
	 * Initialize the View fields, which are all stored as WeakReferences to enable garbage collection.
	 */
	private void initializeViewFields() {
		// Store the ProgressBar in a field for fast access.
		mLoadingProgressBar = new WeakReference<>((ProgressBar) mActivity.get().findViewById(R.id.progressBar_loading));

		// Store the EditText that holds the urls entered by the user
		// (if any).
		mUrlEditText = new WeakReference<>((EditText) mActivity.get().findViewById(R.id.url));

		// Store the linear layout displaying URLs entered.
		mLinearLayout = new WeakReference<>((LinearLayout) mActivity.get().findViewById(R.id.linearLayout));
	}

	/**
	 * Reset the non-view fields (e.g., URLs and counters) and redisplay linear layout.
	 */
	...
	/**
	 * Add whatever URL has been entered into the text field if that URL is valid when user presses the "Add URL" button
	 * in the UI.
	 */
	public void addUrl() {
		...
		if (URLUtil.isValidUrl(url)) {
			...
			mUrlList.add(mUrlEditText.get().getText().toString());

			// (Re)display all the URLs.
			displayUrls();
		} else
			Utils.showToast(mActivity.get(), "Invalid URL " + url);
	}
	...
	private void removeUrl(String url) {
		...
		if (mUrlList.contains(url)) {
			...
			Log.w(TAG, "RemoveUrl() - passed URL (" + (url == null ? "null" : url.toString()) + ") is not in URL list.");
		}
	...
	private void displayUrls() {
		...
		for (String url : mUrlList) {
			...
			urlTextView.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
	...
	/**
	 * Start all the downloads. Plays the role of the "Template Method" in the Template Method pattern.
	 */
	public void startDownloads() {
		// Hide the keyboard.
		Utils.hideKeyboard(mActivity.get(), mUrlEditText.get().getWindowToken());

		if (mUrlList.isEmpty())
			Utils.showToast(mActivity.get(), "no images provided");
		else {
	...
	/**
	 * Start a download. Plays the role of a "Primitive Operation" (aka "Hook Method") in the Template Method pattern,
	 * which is needed since the means for passing requests to a Started Service are different than for a Bound Service.
	 */
	...
	public void doResult(int requestCode, int resultCode, Bundle data) {
		...
		if (resultCode == Activity.RESULT_CANCELED)
			...
			Log.d(TAG, "received image at URI " + ReplyMessage.getImagePathname(data));
	...
	/**
	 * Launch an Activity to display all the images that were received successfully if all downloads are complete.
	 */
	private void tryToDisplayImages(Bundle data) {
		...
		if (allDownloadsComplete()) {
			...
			if (file.isDirectory() && file.listFiles() != null && file.listFiles().length > 0) {
				...
				final Intent intent = DisplayImagesActivity.makeIntent(mDirectoryPathname);

				Log.d(TAG, "starting DisplayImageActivity at " + mDirectoryPathname);
	...
	private void handleDownloadFailure(Bundle data) {
		...
		final Uri url = ReplyMessage.getImageURL(data);

		Utils.showToast(mActivity.get(), "image at " + url.toString() + " failed to download!");
	...
	public void deleteDownloadedImages() {
		...
		Utils.showToast(mActivity.get(), fileCount + " downloaded image" + (fileCount == 1 ? " was" : "s were") + " deleted.");
	...
	/**
	 * A helper method that recursively deletes files in a specified directory.
	 */
	private Integer deleteFiles(String directoryPathname, int fileCount) {
		...
		for (File f : files) {
			if (f.isDirectory())
				fileCount += deleteFiles(f.toString(), fileCount);
			Log.d(TAG, "deleting file " + f.toString() + " with count " + fileCount);
			++fileCount;
			f.delete();
		}
	...
	/**
	 * Called by the ImageOps constructor and after a runtime configuration change occurs to finish the initialization
	 * steps.
	 */
	public void onConfigurationChange(MainActivity activity) {
		...
		if (mServiceResultHandler != null) {
			((ServiceResultHandler) mServiceResultHandler).onConfigurationChange(mActivity.get());
		}
