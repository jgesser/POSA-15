...
/**
 * A thin facade around an Android Message that defines the schema of a request from the Activity to the Service.
 */
public class RequestMessage extends RequestReplyMessageBase {
	/**
	 * Constructor is private to ensure the makeRequestMessage() factory method is used.
	 */
	...
	/**
	 * Factory method creates a RequestMessage to return to the Activity with information necessary to download an
	 * image.
	 */
	public static RequestMessage makeRequestMessage(int requestCode, Uri url, String directoryPathname, Messenger replyMessenger) {
		// Create a RequestMessage that holds a reference to a Message
		// created via the Message.obtain() factory method.
		RequestMessage requestMessage = new RequestMessage(Message.obtain());
