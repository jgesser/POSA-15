...
/**
 * The main Activity that prompts the user for URLs of images to download concurrently via various implementations of
 * DownloadImages*Service and view via the DisplayImagesActivity. Also allows the user to delete downloaded images.
 * Extends LifecycleLoggingActivity so its lifecycle hook methods are logged automatically and implements ServiceResult
 * so that the onServiceResult() hook method is called back when an image has been downloaded. This implementation uses
 * the RetainedFragmentManager class to handle runtime reconfigurations robustly. As a result, MainActivity plays the
 * role of the "Caretaker" in the Memento pattern.
 */
public class MainActivity extends LifecycleLoggingActivity implements ServiceResult {
	/**
	 ...
	 * layout initialization and runtime configuration changes.
	 *
	 * @param Bundle
	...
	/**
	 * Called by the Android Activity framework when the user preses the "Download and Display Image(s)" button in the
	 * UI.
	 *
	 * @param view
	 *            The view.
	...
	/**
	 * Delete the previously downloaded pictures and directories when user presses the "Delete Downloaded Image(s)"
	 * button in the UI.
	 */
	...
	/**
	 * Hook method called back by the ServiceResultHandler when a Service that's been launched finishes, giving the
	 * requestCode it was started with, the resultCode it returned, and any additional result data returned from the
	 * service.
	 */
	@Override
	public void onServiceResult(int requestCode, int resultCode, Bundle data) {
		// Handle the results.
		mImageOps.doResult(requestCode, resultCode, data);
	}
}
