...
/**
 * A Bound Service that concurrently downloads an image requested via a Message passed to a Request Messenger, stores
 * the image in a file on the local device, and returns the URI to the downloaded image file back to the Activity via
 * the Reply Messenger passed with the original Message.
 */
public class DownloadImagesBoundService extends LifecycleLoggingService {
	/**
	 * A RequestHandler that handles request Messages send from the Activity.
	 */
