...
/**
 * This class handles messages sent from an Activity in a pool of threads managed by the Java ExecutorService.
 */
class RequestHandler extends Handler {
	...
	/**
	 * Hook method called back when a request message arrives from an Activity. The Message it receives contains the
	 * Messenger used to reply to the Activity and the URL of the image to download. This image is stored in a local
	 * file on the local device and image file's URI is sent back to the MainActivity via the Messenger passed with the
	 * message.
	 */
	@Override
	public void handleMessage(Message message) {
		...
		final Messenger replayMessenger = requestMessage.getMessenger();
		...
		final Runnable downloadImageAndReply = new Runnable() {
			...
			public void run() {
				...
				// replyMessenger.
				// TODOne -- you fill in here.
				sendPath(replayMessenger, downloadedImageURL, imageURL, requestCode);
			}
		};
	...
	/**
	 * Send the @a pathToImageFile, @a url, and @a requestCode back to the Activity via the @a messenger.
	 */
