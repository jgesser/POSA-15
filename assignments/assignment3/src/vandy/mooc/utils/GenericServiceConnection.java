 ...
 *
 * @brief This class provides a generic framework for defining a ServiceConnection object to an AIDLInterface that
 *        resides in a Bound Service. It uses Java reflection to factor out common behavior that is otherwise written in
 *        a tedious and error-prone way for each ServiceConnection object.
 *
...
public class GenericServiceConnection<AIDLInterface extends android.os.IInterface> implements ServiceConnection {
	/**
	 * The following are dependent on code generated by the AIDL compiler, so if that generated code were to change
	 * (which is unlikely) we'd need to update this code.
	 */
	...
	/**
	 * Reference to the AIDL interface object after the client has finished binding to the Bound Service.
	 */
	 ...
	 * 
	 * @param aidl
	 *            the AIDL Interface class object which must match the generic parameter.
	 */
	...
	/**
	 * Hook method called back by the Android Service framework after connection is established to a Bound Service.
	 */
	...
	/**
	 * Called if the Bound Service crashes and is no longer available. The ServiceConnection will remain bound, but the
