...
public class WeatherOpsImpl implements WeatherOps {
	...
	/**
	 * This GenericServiceConnection is used to receive results after binding to the WeatherServiceSync Service using
	 * bindService().
	 */
	private GenericServiceConnection<WeatherCall> mServiceConnectionSync;

	/**
	 * This GenericServiceConnection is used to receive results after binding to the WeatherServiceAsync Service using
	 * bindService().
	 */
	private GenericServiceConnection<WeatherRequest> mServiceConnectionAsync;

	/**
	 * This Handler is used to post Runnables to the UI from the mWeatherResults callback methods to avoid a dependency
	 * on the Activity, which may be destroyed in the UI Thread during a runtime configuration change.
	 */
	private final Handler mDisplayHandler = new Handler();

	/**
	 * The implementation of the WeatherResults AIDL Interface, which will be passed to the Weather Web service using
	 * the WeatherRequest.expandWeather() method.
	 *
	 * This implementation of WeatherResults.Stub plays the role of Invoker in the Broker Pattern since it dispatches
	 * the upcall to sendResults().
	 */
	private final WeatherResults.Stub mWeatherResults = new WeatherResults.Stub() {
		/**
		 * This method is invoked by the WeatherServiceAsync to return the results back to the WeatherActivity.
		 */
		@Override
		...
		/**
		 * This method is invoked by the WeatherServiceAsync to return error results back to the WeatherActivity.
		 */
		@Override
	...
	/**
	 * (Re)initialize the non-view fields (e.g., GenericServiceConnection objects).
	 */
	...
	public void expandCurrentWeatherSync(View v) {
		...
		if (weatherCall != null) {
			...
			new AsyncTask<String, Void, WeatherData>() {
				...
				/**
				 * Retrieve the expanded weather results via a synchronous two-way method call, which runs in a
